...
def make_toy_scenarios(
    ...
    for scenario_id in range(num_scenarios):
        ...
        spatial_noise = np.clip(
            rng.normal(1.0, 0.025, (horizon, len(feeder.buses), len(feeder.phases))),
            0.92,
            1.08,
        )
        active_load = (
            temporal_load[:, None, None]
            * feeder.base_active_load_mw[None, :, :]
            * spatial_noise
        )
        ...
        reactive_load = active_load * reactive_ratio[None, :, :]

        cloud = np.clip(1.0 + _smooth_noise(rng, horizon, 0.06), 0.65, 1.15)
        pv_available = (
            solar_scale
            * solar_shape[:, None, None]
            * cloud[:, None, None]
