...
def train_direct_generator(
    ...
    for epoch in range(config.epochs):
        print(f"DFL epoch {epoch + 1}/{config.epochs}: solving planning model...", flush=True)
        ...
        print(
            f"DFL epoch {epoch + 1}/{config.epochs}: planning status={plan.status}, "
            f"objective={plan.objective:.6g}, seconds={plan.solve_time_seconds:.1f}",
            flush=True,
        )
        ...
        if plan.feasible:
            ...
            print(
                f"DFL epoch {epoch + 1}/{config.epochs}: validation "
                f"status={validation.status}, objective={validation.objective:.6g}, "
                f"seconds={validation.solve_time_seconds:.1f}",
                flush=True,
            )
        else:
            validation = plan
    ...
    final_validation_scenarios = _validation_scenarios(
        observed_pool,
        epoch=0,
        batch_size=config.final_validation_size,
    )
